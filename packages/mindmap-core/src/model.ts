...
export interface Zone {
  ...
  color: string | null;
  /**
   * 열 모드(칸반) — 이 프레임 안의 카드가 세로로 쌓이고 폭이 열에 맞춰진다.
   * post-dc 순수 추가라 **값이 있을 때만** 직렬화된다(옛 문서·골든 무회귀).
   *
   * 순서를 배열로 들지 않는 이유는 협업이다: 끊긴 채 두 사람이 카드를 옮기면
   * 배열 필드는 한쪽이 통째로 사라진다(#332). 대신 **y 좌표가 곧 순서**다.
   */
  stack?: 'column';
}
