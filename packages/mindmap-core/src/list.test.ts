...
describe('listBackspaceOp — 마커 안에서의 Backspace', () => {
  it('내용이 있는 최상위 마커 뒤에서는 마커만 없앤다', () => {
    expect(listBackspaceOp('• 하나', 2)).toEqual({ kind: 'op', op: { type: 'toggle', kind: 'ul' } });
  });

  it('**빈** 최상위 항목은 줄까지 통째로 지운다 (빈 줄이 남아 캐럿이 떨어지지 않게)', () => {
    const t = '1. 하나\n2. ';
    expect(listBackspaceOp(t, t.length)).toEqual({ kind: 'edit', edits: [{ at: 5, remove: 4, insert: '' }] }); // '\n2. '
    // 앞에 줄이 없으면(첫 줄) 지울 줄바꿈도 없다 — 마커만 없앤다
    expect(listBackspaceOp('2. ', 3)).toEqual({ kind: 'op', op: { type: 'toggle', kind: 'ol' } });
  });

  it('가운데 빈 항목을 지우면 뒤 번호가 다시 매겨진다', () => {
    const t = '1. 하나\n2. \n3. 셋';
    const act = listBackspaceOp(t, 9)!; // '2. ' 끝
    expect(act.kind).toBe('edit');
    const edits = (act as { kind: 'edit'; edits: { at: number; remove: number; insert: string }[] }).edits;
    const out = [...edits].sort((x, y) => y.at - x.at).reduce((acc, e) => acc.slice(0, e.at) + e.insert + acc.slice(e.at + e.remove), t);
    expect(out).toBe('1. 하나\n2. 셋');
  });

  it('들여쓴 마커 뒤에서는 한 단계 내어쓴다', () => {
    const t = '1. 하나\n  a. ';
    expect(listBackspaceOp(t, t.length)).toEqual({ kind: 'op', op: { type: 'indent', dir: -1 } });
  });

  it('마커 중간(들여쓰기 공백 안 포함)에서도 같게 동작한다', () => {
    const t = '  a. 하나';
    expect(listBackspaceOp(t, 1)).toEqual({ kind: 'op', op: { type: 'indent', dir: -1 } }); // 들여쓰기 공백 안
    expect(listBackspaceOp(t, 3)).toEqual({ kind: 'op', op: { type: 'indent', dir: -1 } }); // 숫자/기호 뒤
  });
