...
"Microsoft Store 채널(MSIX)" 참고). 결정된 것:

| 항목 | 값 |
| --- | --- |
| 계정 종류 | **회사 계정** — 그리오를 위한 **본인 개인사업자 명의**(고용주 아님) |
| 회사명(법적) | **`그리오(Geurio)`** — 사업자등록증 상호 그대로. 홈택스가 한글 상호만 받고 영문은 괄호 병기를 요구해서 이 꼴이 됐다 |
| 발행자 표시 이름 | **`Geurio`** — Store 페이지에 보이는 이름. 법적 이름과 달라도 된다(§1 끝) |
| 앱 이름(예약) | **`그리오 (Geurio)`** |
| 등록 정보 언어 | **한국어 + 영어** |
...
### 회사명과 발행자 표시 이름은 다른 값이다

| | 값 | 성질 |
| --- | --- | --- |
| 회사명(법적) | `그리오(Geurio)` | 심사가 **사업자등록증과 글자 하나까지 대조**한다. 오탈자·약어가 있으면 반려된다 |
| 발행자 표시 이름 | `Geurio` | **Store 페이지에 보이는 이름.** 법적 이름과 **달라도 된다** — Microsoft 답변: "같게 두든 다르게 두든 되지만 대체로 비슷하게 두는 것이 좋다" |

`Geurio`를 고른 이유: 법적 이름의 괄호 안 표기 그대로라 "대체로 비슷하다"에
들어맞고, 전 세계 Store 사용자에게 읽히며, 도메인·아이콘·브랜드 마크와 한 이름으로
묶인다(앱 이름 `그리오 (Geurio)`와 발행자 이름이 겹쳐 보이는 것도 피한다).

**둘 다 등록 단계에서 확정한다.**

- 회사명은 **등록증에 찍힌 문자열을 복사해서** 붙여 넣는다 — 반각 `(`인지 전각
  `（`인지, 괄호 앞에 띄어쓰기가 있는지가 다르면 "정확히 일치"에서 어긋난다.
- 발행자 표시 이름을 나중에 바꾸면 **계정이 재심사를 받고, 앱을 새 이름으로 다시
  패키징해 재제출**해야 한다. 그 값은 MSIX 매니페스트(`publisherDisplayName`)에
  박히므로 우리 빌드까지 다시 돌려야 한다. 그래서 처음에 정한다.
- 상표: 남이 상표권을 가진 이름을 쓰면 **계정이 닫힐 수 있다**는 것이 Microsoft의
  경고다. `Geurio`는 조어이고 도메인을 보유 중이라 위험은 낮게 본다.
...
appx:
  ...
  publisherDisplayName: Geurio                   # 우리가 정한 발행자 표시 이름(§1 끝)
```
...
- **정책 전문**(`Microsoft Store Policies`)은 `learn.microsoft.com`이 이
  ...
  다르면 **화면을 따른다**.
- 발행자 표시 이름을 **한글로 둘 수 있는지**는 문서에 명시가 없다(한국 판매자가
  많아 받을 것으로 보지만 확인하지 못했다). 우리는 `Geurio`로 정했으므로 이
  물음을 비켜 간다.
