...
"Microsoft Store 채널(MSIX)" 참고). 결정된 것: **개인 계정** · 예약할 이름
...
Partner Center의 앱·게임 페이지에서 `Get started for free`로 시작한다.
개인 계정은 본인 이름이 **발행자로 Store에 표시**되므로, 표시될 이름을
등록 단계에서 확인한다(나중에 바꾸는 것은 지원 요청이 필요하다).

> 신원 확인 절차는 최근 개편돼 계속 바뀌고 있다 — 화면 안내를 따른다.
> 이 문서를 쓴 시점에 확인한 것은 "등록비가 없다"는 사실까지다.
...
- **정책 전문**(`Microsoft Store Policies`)은 `learn.microsoft.com`이 이
  ...
  다르면 **화면을 따른다**.
- 개인 계정의 신원 확인 절차는 최근 개편이 잦아 확정하지 못했다.
