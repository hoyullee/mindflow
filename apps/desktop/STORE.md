...
> 계정 종류를 회사로 정한 근거는 Microsoft의 정의다 — **개인**은 "Store를 통한
> 앱 배포가 자신의 사업·직업·업무와 **관련되지 않은**" 경우(취미·학습·비상업),
> **회사**는 "**관련된**" 독립 개발자·프리랜서와 법인·단체다. 그리오는 실제
> 서비스로 운영되므로 회사 쪽이다. 계정 종류는 나중에 바꾸기 어려운 값이니
> 처음에 맞게 고른다.
...
| 항목 | 내용 |
| --- | --- |
| 로그인 계정 | **업무 계정(Entra ID)** 또는 개인 Microsoft 계정. Entra ID 가입은 **회사 계정에서만** 된다 |
| 회사 정보 | **DUNS 번호** 또는 공식 서류 |
| 연락처 | **조직 도메인의 업무 이메일**(`@wantedlab.com`) |

확인은 세 갈래로 돈다 — **실사(due diligence, 이게 통과해야 다음이 진행된다)** ·
**사업체 확인** · **재직 확인**. 수동 검토는 **영업일 2~5일**이 걸린다. 등록에 적은
도메인과 이메일 도메인이 어긋나면 **도메인 확인** 서류를 더 요구할 수 있다.

> 회사 이름으로 내는 일이고 회사 정보(DUNS·서류)와 재직 확인이 필요하므로,
> 사내에 미리 알리고 필요한 값을 받아 두는 편이 빠르다. 개인 계정 경로였다면
> 개인 Microsoft 계정 + **신분증·셀피** 확인으로 끝나지만, 그 종류는 위
> 인용한 정의상 우리에게 맞지 않는다.
...
- 회사 계정의 **발행자 표시 이름이 무엇에서 정해지는지**는 문서에 없었다
  (등록 화면에서 확인해야 한다).
- 계정 종류를 등록 후에 **바꿀 수 있는지**는 확인하지 못했다 — 바꿀 수 없다고
  보고 처음에 맞게 고르는 편이 안전하다.
