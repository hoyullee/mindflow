...
//
// Protocol (two message kinds, both plain objects — `BroadcastChannel`
...
//   { kind: 'update', update: Uint8Array } — a Yjs update (either a genuine
...
type Message = { kind: 'sync-request' } | { kind: 'update'; update: Uint8Array };
...
export class BroadcastChannelProvider implements CollabProvider {
  ...
  connect(docId: string, ydoc: YDoc): void {
    ...
    // Ask any already-open tabs for the current doc's full CRDT state so this
    // (newly-opened) tab catches up even if it missed earlier updates.
