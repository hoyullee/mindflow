 ...
 *
 * 조용히 적용됐든(랜딩·약관·백그라운드 탭), 사용자가 설정 › 「버전 확인」에서 눌러
 * 적용했든, 옆 탭이 적용해 이 탭이 덩달아 리로드됐든 경로와 무관하게 같은 알림이
 * 뜬다 — "리로드된 이유"를 알려 주는 게 목적이라 원인이 무엇이든 사용자에게는 같은
 * 사실이다.
 *
 * 물어보는 게 아니라 **알리는** 것이므로 버튼이 없고 스스로 사라진다. 적용 **전에**
 * 말을 거는 자리(홈 LNB의 새 버전 알림)와 역할이 다르다 — 그쪽은 누를 것이 있다.
 */
