...
export interface WorkspaceData {
   ...
   *
   * 처음에는 이 두 값을 기기(localStorage)에만 두었는데, 그래서 같은 계정으로
   * 크롬과 설치형 앱에 함께 로그인하면 **한쪽에만 알림이 왔다** — 특히 구글 일정
   * 알림은 기본이 꺼짐이라 크롬에서 켠 사람도 앱에서는 꺼진 채였다. "내가 건 알림을
   * 받는다"는 계정의 뜻이지 기기의 뜻이 아니므로 여기로 옮긴다.
   *
   * 기기에 남는 것은 **정말로 기기마다 다른 것 하나**뿐이다: OS 알림 권한(그건
   * 브라우저·OS가 들고 있어 우리가 옮길 수 없다). localStorage의 두 키는 이제
   * 이 값의 **캐시**다(첫 페인트·에디터가 블롭을 읽기 전).
   *
   * 예전 블롭에는 없으므로 선택("아직 고른 적 없음") — 그때는 각 기기가 실제로
   * 고른 값이 있으면 그것을 계정으로 올려 준다(`explicitReminderPrefs`).
   */
  reminders?: {
    /** 알림을 받을까(없으면 켜짐 — `reminderPrefs.ts`의 근거). */
    on?: boolean;
    /** 구글 일정에 걸린 알림도 **우리가** 띄울까(없으면 꺼짐 — 구글이 이미 보낸다). */
    google?: boolean;
  };
}
