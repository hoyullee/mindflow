...
export function useEditorState(): EditorController {
  ...
  const onRemoteDoc = useCallback((d: Doc) => {
    /**
     * **공책에는 원격 판을 적용하지 않는다**(제보: 공책이 통째로 빈 화면).
     *
     * 공책은 실시간 공동 편집을 붙이지 않기로 한 문서라(`CLAUDE.md`) **CRDT 바인딩에
     * 페이지가 아예 없다** — `yDocToDoc`이 돌려주는 판에는 `pages`가 담기지 않는다.
     * 그런데 연결 자체는 종류를 가리지 않고 맺힌다(접속자 얼굴을 그 채널에서 받는다).
     * 그래서 **두 번째 사람이 같은 공책을 여는 순간** 원격 갱신이 한 번 오고, 그
     * 판에는 페이지가 없으므로 이쪽 화면이 통째로 비었다(실측 제보: 로컬 사본은
     * 2쪽·3KB로 멀쩡한데 화면만 `0쪽`. 다른 탭을 닫기 전에는 새로고침해도 되풀이됐다).
     *
     * 문서를 읽는 다른 경로는 전부 `parseDoc`을 지나 페이지를 한 장 이상 세워 주는데
     * **이 경로만 날것을 그대로 `setDoc` 한다** — 그 비대칭이 이 사고의 뿌리다.
     *
  ...
  const refreshFromServer = useCallback(async (): Promise<void> => {
    ...
    /**
     * ②-b **글을 쓰고 있는 중이면 갈아 끼우지 않는다**(제보: 다른 앱을 쓰다 돌아오면
     * 문서를 다시 불러오느라 깜빡이고 커서가 풀린다).
     *
     * 위의 ①②는 "내가 고친 것이 남아 있는가"를 본다 — 저장이 막 끝난 상태에서는
     * 둘 다 통과한다. 그런데 그 순간에도 사람은 **그 줄에 커서를 두고 있을 수 있고**,
     * 서버 판으로 갈아 끼우면 편집 박스가 통째로 다시 그려져(`docEpoch`) 초점과
     * 캐럿이 사라진다. 남이 바꾼 것이 실제로 있더라도, 쓰고 있는 사람의 손에서
     * 글을 빼앗는 것보다 **다음 계기까지 미루는** 편이 낫다(창을 떠났다 오거나
     * 초점을 놓으면 그때 받는다).
     */
    if (typeof document !== 'undefined') {
      const active = document.activeElement as HTMLElement | null;
      // **속성으로 본다** — `isContentEditable`은 jsdom이 구현하지 않아(실측: `undefined`)
      // 그 값으로 가르면 테스트에서는 가드가 통째로 죽는다. 속성은 양쪽에서 같다.
