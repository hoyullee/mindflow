...
/**
 * 본문에서 **무엇을 할 수 있는가**(머리줄 배지) — 스펙 §3.2-3.
 *
 * 칸반만 한 가지 편집(열 이동)을 허용하고 나머지는 보기 전용이다. 편집 권한이 없으면
 * 칸반도 보기 전용이다 — 배지는 "여기서 되는 일"을 말하는 자리이지 문서의 성질을
 * 말하는 자리가 아니다.
 */
export function embedRuleLabel(kind: EmbedKind, canEdit: boolean): string {
  return kind === 'kanban' && canEdit ? '열 이동만' : '보기 전용';
}
