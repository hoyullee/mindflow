...
describe('Home', () => {
  ...
  it('map cards show the LAST-EDITED time (updatedAt), not the legacy "최근 항목:내 맵" line', async () => {
    ...
    await waitFor(() => expect(within(card).getByText('변경일 · 2시간 전')).toBeTruthy());
    // 옛 문구는 사라져야 한다
    expect(within(card).queryByText(/최근 항목:/)).toBeNull();
    expect(within(card).queryByText('내 맵')).toBeNull();
    // 정확한 일시는 툴팁으로
    expect(within(card).getByText('변경일 · 2시간 전').getAttribute('title')).toMatch(/^\d{4}\. \d{1,2}\. \d{1,2}\. \d{2}:\d{2}$/);
  });
