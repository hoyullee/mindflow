      ...
      <div style={{ position: 'relative', height: HOUR_ROW * 24, display: 'flex', flexDirection: 'column' }}>
        ...
        {timeline.blocks.map((b) => {
          ...
          return (
            ...
            >
              <span style={{ fontSize: n > 2 ? 11 : 12.5, fontWeight: 800, letterSpacing: '-.015em', color: 'var(--mf-text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', ...declinedStyle(b.entry) }}>{roomMark(b.entry)}{b.entry.title}</span>
              {n <= 2 && (
