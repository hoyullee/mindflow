...
import type { GoogleEventDraft } from './googleCalendar';
import type { NewEventTarget } from './NewEventModal';

/** 우리 말(`note`) → 구글 말(`description`). 그 밖은 이름이 같다. */
export function inputToGoogleDraft(input: CalendarEventInput): GoogleEventDraft {
  return {
    title: input.title,
    allDay: input.allDay,
...
export function submitNewEvent(input: CalendarEventInput, target: NewEventTarget, sinks: NewEventSinks): Promise<string | null> {
  return target.kind === 'google' ? sinks.createGoogle(target.calendarId, inputToGoogleDraft(input)) : sinks.createGeurio(input);
}
