...
// 겹쳐 한 덩어리로 보인다(실제로 그렇게 겹쳤다). 루트 아래쪽에 세로로 쌓고,
// 자유 도형은 왼쪽 / 메모는 오른쪽으로 갈라 놓는다. 겹치지만 않으면 되고 위치는
// 사용자가 옮기면 된다 — 트리 크기를 미리 알 수 없어 완벽한 자리는 없다.
/** 메모 폭은 앱에서 삽입할 때와 같은 180(`mutations.addFloatItem`). x/y는 좌상단. */
const MEMO_X = 240;
const MEMO_TOP = 220;
const MEMO_GAP = 110;
 ...
 *
 * `toMarkdown`(코어)이 쓰는 세 가지를 **되읽는다** — 내보낸 `.md`를 다시 가져오면
 * 트리뿐 아니라 노트·자유 도형·메모까지 복원된다(예전엔 전부 잃거나 엉뚱한 가지로
 * 들어왔다):
 *  - `> 텍스트` → 바로 위 노드의 `note`
 *  - `## 개별 주제` 섹션의 항목 → `free: true` 노드(트리에 붙지 않는 자유 도형)
 *  - `## 메모` 섹션의 항목 → 플로트(메모 카드)
 *
 * 그 두 제목(`toMarkdown`이 쓰는 문구 그대로)만 섹션으로 본다. 다른 `##`은 예전처럼
 * 무시하고, 그 아래 불릿은 현재 섹션에 계속 쌓인다 — 외부에서 만든 마크다운의 해석을
 * 바꾸지 않기 위해서다.
 */
export function parseOutline(text: string, fallbackTitle: string): StoredDoc | null {
  ...
  let itemCount = 0;
  let freeCount = 0;
  /** 현재 섹션의 부모 스택. 트리는 root에서, 자유 도형 섹션은 매 항목이 새 뿌리다. */
  let stack: { depth: number; id: string }[] = [{ depth: 0, id: 'root' }];
  /** 방금 만든 노드 — `>` 노트가 붙을 대상. */
  let lastNodeId: string | null = null;

  const indentOf = (ws: string): number => Math.floor(ws.replace(/\t/g, '  ').length / 2) + 1;

  for (const ln of lines) {
    // `## …` 먼저 본다 — 아래 `#` 정규식은 `#` 뒤 공백을 요구하므로 `##`과 겹치지 않지만,
    // 섹션 전환을 제목보다 먼저 처리해야 순서에 휘둘리지 않는다.
    const h2 = ln.match(/^##\s+(.+)/);
    if (h2) {
    ...
    if (h1 && itemCount === 0) {
      root.text = (h1[1] ?? '').trim();
      lastNodeId = 'root'; // 루트 노트(`  > …`)가 바로 뒤에 온다
      continue;
    }
    ...
    if (note) {
      ...
      if (target && body) target.note = target.note ? `${String(target.note)}\n${body}` : body;
      continue;
    }
  ...
  // 아무것도 못 찾았을 때만 실패로 본다(메모만 있는 파일도 가져올 게 있는 것이다).
  if (!itemCount && !memos.length) return null;
  const floats = memos.map((t, i) => ({ id: 'fm' + (i + 1), x: MEMO_X, y: MEMO_TOP + i * MEMO_GAP, w: 180, text: t }));
