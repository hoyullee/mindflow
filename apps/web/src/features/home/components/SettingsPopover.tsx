  ...
  return (
    <div style={{ position: 'relative', marginBottom: 10 }}>
      ...
      >
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            {state.nameEditing ? (
              <input
                ...
                aria-label="프로필명 입력"
                maxLength={20}
                style={{ width: '100%', height: 32, border: '1px solid #f0663f', borderRadius: 8, background: '#fff', color: '#33281f', fontFamily: 'inherit', fontSize: 15, fontWeight: 700, padding: '0 9px', outline: 'none', boxSizing: 'border-box' }}
              />
            ) : (
              <>
        ...
        <div style={{ padding: 6 }}>
          {/* profile-name rename — moved here from the 설정 modal */}
          {state.nameEditing ? (
            <div
              className="menu-row"
              role="button"
              tabIndex={0}
              // preventDefault so this mousedown doesn't blur the input first
              onMouseDown={(e) => e.preventDefault()}
              onClick={controller.onNameEditDone}
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  e.preventDefault();
                  controller.onNameEditDone();
                }
              }}
              style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '10px 12px', borderRadius: 9, fontSize: 13, fontWeight: 600, cursor: 'pointer', color: '#2f9e63' }}
            >
            ...
            <div
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  e.preventDefault();
                  controller.startNameEdit(e as unknown as MouseEvent);
                }
              }}
