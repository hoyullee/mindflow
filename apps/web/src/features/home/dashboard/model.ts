...
/** 세로 최대 행 수 — 원본은 3행까지였고 요청으로 4행(N×4), 다시 5행(N×5)을 열었다.
 *  5행은 152px×5 + 간격이라 노트북 한 화면보다 높지만 대시보드는 세로로 스크롤하고,
 *  달력 위젯의 월간 + 옆 패널(시간표)은 그만한 높이를 실제로 쓴다(요청). */
export const DASH_ROWS_MAX = 5;
/** 고르기 쉬운 **선택지**(피커·크기 순환·메뉴). 자유 리사이즈는 이 목록에 없는
 *  조합도 만들 수 있으므로 저장값 검증은 `isValidSize`가 맡는다. */
export const DASH_SIZES = ['1x1', '2x1', '1x2', '2x2', '3x2', '4x2', '1x4', '3x3', '4x3', '2x4', '3x4', '4x4', '3x5', '4x5'] as const;
