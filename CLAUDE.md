...
- ✅ **SNS 가입 이메일의 이메일 회원가입 차단(제보)** Google로 가입한 이메일로 이메일 회원가입을 시도하면 가입이 진행되는 듯 **인증번호 화면까지 넘어가는데 코드는 영영 오지 않았다**. 원인은 Supabase `auth.signUp`의 이메일 열거 방지 — 이미 가입된 주소에도 **성공을 돌려주고 메일은 보내지 않는다**(유일한 단서가 `identities`가 빈 배열인 가짜 user). 수리 2겹: ① 가입 **전에** 그 이메일의 로그인 수단을 조회(`email_signin_providers` RPC, 0013 — 0008과 같은 트레이드오프로 이메일 열거 허용, 공급자 이름만 반환)해 이미 가입돼 있으면 막고 **어떻게 가입했는지**까지 안내(소셜이면 "Google 계정으로 가입한 이메일 — 아래 Google로 계속하기", 이메일이면 로그인 탭 유도 링크). ② RPC 미배포·네트워크 실패로 확인 불가(`null`)여도 어댑터가 `identities: []`를 감지해 '이미 가입됨' 오류로 바꾸므로 **막다른 인증 화면으로는 절대 넘어가지 않는다**. UI는 비밀번호 찾기의 미가입 툴팁과 같은 톤의 콜아웃(`signupBlockedCalloutStyle`), 이메일을 고치면 해제. 어댑터 7 + 로그인 UI 5 테스트, 데모 모드 가입 흐름 실브라우저 무회귀 확인(차단 경로는 Supabase 전용이라 유닛/통합 테스트로 검증)
