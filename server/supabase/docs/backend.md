...
2. **OAuth 동의 화면**(개편 후 `Google 인증 플랫폼` → `데이터 액세스`) → 범위 →
   `.../auth/calendar.events`와 `.../auth/calendar.calendarlist.readonly`를 **둘 다
   추가**한다(§1d에서 만든 그 동의 화면이다 — 클라이언트 ID도 그대로 쓴다.
   `VITE_GOOGLE_CLIENT_ID`는 이미 있다).
...
**저장이 안 될 때**: ① 설정·머리에 `다시 연결`이 떴는가(= 승인된 스코프가 모자라다 —
동의 화면에서 권한을 모두 허용해야 한다) ② 그 캘린더에 `보기 전용` 표시가 있는가
③ 네트워크 탭의 `PATCH`/`POST` 상태 코드 — 403은 권한, 412는 그 사이 구글에서 바뀐 것.

### 하지 않은 것

- **반복 규칙 만들기·고치기**(RRULE) · 참석자·Meet·알림 — 구글에서 하도록 링크로 보낸다.
