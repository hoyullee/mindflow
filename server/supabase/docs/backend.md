...
- 고르면 다른 두 나라의 공휴일 id를 `calendars`·`extra`에서 빼고 그 나라를 넣는다.
  구독하지 않은 국가는 `extra`로 들어간다(위 규칙 그대로 — `events.list`로 읽힌다).
- 저장된 값이 없으면 **목록에 든 공휴일 캘린더**가 지금 국가를 말한다(옛 블롭 무회귀).
- UI는 **연결된 뒤에만** 그린다(연결 전에는 고를 수 있는 척만 하게 된다), 그리고 그
  캘린더는 목록에서 ✕로 뺄 수 없다 — 그 자리는 이 세그먼트가 맡으므로 빼면 국가 설정만
  남아 둘이 어긋난다.
