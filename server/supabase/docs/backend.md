...
> 드러나는 것이라, `다시 연결` 툴팁·설정 부제가 "한 시간마다 만료되고, 다시 연결하면
> 고른 캘린더가 그대로 이어진다"고 밝힌다. 진짜로 없애려면 서버 쪽 auth-code +
> refresh token 흐름(Edge Function + 구글 데이터의 서버 보관)이 필요하고, 그건 지금
> 구글 검수 중인 §4·§5 방침까지 함께 고쳐야 하는 별건이다.
...
1. 앱: `signInWithOAuth({ skipBrowserRedirect: true, redirectTo: <위 주소> })`로
   **시작 주소만** 받아 시스템 브라우저에서 연다. 이때 PKCE verifier가 **앱의**
   저장소에 남는다.
2. 브라우저: 동의 → Supabase 콜백 → `/auth/desktop?code=…`.
3. 브라우저: 그 **인가 코드**를 `geurio://auth?code=…`로 앱에 넘긴다. 세션을 세우지
   않고, 그 사람이 웹에서 따로 로그인해 둔 세션도 건드리지 않는다.
4. 앱: `exchangeCodeForSession(code)` — 자기 verifier로 교환해 세션을 세운다.

**코드가 주소에 실려 지나가는 것**이 이 방식의 약한 고리인데, PKCE 코드는 그
verifier를 가진 클라이언트(=앱)만 쓸 수 있고 한 번 쓰면 죽는다. 세션은 애초에 앱
하나에만 선다.

### 함정 — 세션(갱신 토큰)을 넘기면 안 된다 (제보로 확인)

첫 판은 브라우저에 선 세션의 갱신 토큰을 넘겼고 **매번 실패했다**(앱에
`인증 코드가 올바르지 않거나 만료되었어요`). 원인이 둘이었다.

- 우리 클라이언트는 **PKCE** 흐름이라 콜백은 `?code=`로 오고 그 코드는 **앱의**
  verifier로만 교환된다 → 브라우저는 애초에 세션을 세울 수 없다. 그래서 브라우저가
  넘길 수 있는 세션은 그 사람이 **전에 웹에서 로그인해 둔** 것뿐이었다.
- 뒤이어 부른 `signOut('local')`이 **그 세션을 서버에서 끊었다.** GoTrue의 `local`은
  "이 세션만 로그아웃"이라는 뜻이고(전체 세션을 끊는 `global`의 반대) auth-js가
  `POST /logout?scope=local`을 보낸다 — **저장소만 비우는 것이 아니다.** 앱이
  이어받을 그 토큰이 그 자리에서 폐기됐다.

허용 목록에 넣지 않으면 Supabase가 콜백에서 **사이트 URL로** 되돌려 보낸다 —
브라우저는 로그인이 되지만 앱은 아무것도 받지 못하고 로그인 화면에 남는다(그
경우 브라우저는 "로그인 정보를 받지 못했어요"를 보여 준다).

### 확인
- 앱에서 `Google 계정으로 계속하기` → 시스템 브라우저가 열린다(앱 창이 아니다).
- 브라우저가 `/auth/desktop`에서 "Geurio 앱으로 돌아가세요"를 보여 준다(주소창에
  `?code=`가 **남아 있지 않다** — 엔트리가 낚아채 치운다).
