...
- **매주 되풀이는 하루짜리에만 뜬다**(요청 — 구글의 `근무 위치 수정` 화면과 같은 두
  ...
  - **이미 반복인 근무 위치(회차)를 열면 라디오 대신 한 줄로 알린다** — 구글은 회차에
    규칙을 받지 않고, 반복을 끊는 일은 범위를 되묻는 또 하나의 팝업이 필요하다. 그래서
    PATCH에 규칙을 싣는 것은 `!recurringEventId`일 때뿐이다(`workLocationPatch`의
    셋째 인자).
  - 반복을 **캘린더 설정에서** 정하는 길(구글의 그 화면)은 우리 스코프로 닿지 않는다 —
    그건 여전히 구글 캘린더의 일이다.
