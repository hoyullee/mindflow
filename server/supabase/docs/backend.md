...
- **기본 캘린더에만.** 구글이 이 일정을 다른 캘린더에 받지 않는다. 그래서 진입점은 기본
  ...
  나타나지 않아 고장으로 읽힌다).
- **본문**: `eventType` + `start`/`end`(하루치 — 끝은 배타적인 다음 날) +
  `workingLocationProperties`(`homeOffice` / `officeLocation.label` /
  `customLocation.label`) + `visibility: 'public'` · `transparency: 'transparent'`
  (그 날의 상태라 바쁨으로 잡히면 회의실·한가함 조회에 걸린다). 제목은 **지어 넣지
  않는다** — 구글의 클라이언트가 갈래로 보여 준다.
- **고치기는 갈래·이름만**(`workLocationPatch`) — 날짜는 그대로다(그 날의 상태만 바뀐다).
- **여러 날에 걸친 근무 위치는 손대지 않는다.** 하루만 고치려면 일정을 쪼개야 하는데
  (구글의 클라이언트가 하는 일) 그 규칙을 흉내 내면 남의 달력이 조용히 어그러진다 —
  팝업이 사유를 적고 저장·지우기를 내주지 않는다(반복 일정 삭제를 상세로 미루는 것과
  같은 결). 반복 근무 위치(매주 재택)는 회차가 하루짜리로 오므로 그대로 고칠 수 있다.
