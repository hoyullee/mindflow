...
## 19. 구글 캘린더 겹치기 (PR5) — 서버 없는 연동

일정 화면에 사용자의 **구글 일정을 읽기 전용으로 겹친다.** 마이그레이션도 Edge Function도
없다 — 브라우저가 구글 API를 직접 부르고, 우리는 **아무것도 저장하지 않는다.**
...
로그인은 GIS **ID 토큰** 흐름이라(§1d) API 스코프를 주지 않는다. 캘린더를 읽으려면
`https://www.googleapis.com/auth/calendar.readonly` 스코프의 **액세스 토큰**이 필요하고,
그건 GIS의 다른 API(`google.accounts.oauth2.initTokenClient`)다. 로그인에 스코프를 묶지
않은 것은 구글이 권하는 **점진적 승인**이기도 하다 — 캘린더를 안 쓰는 사람에게 캘린더
권한을 묻지 않는다.

### 배포 전 필요한 것 — **콘솔 작업 + 앱 검수**

`calendar.readonly`는 구글이 **민감(sensitive) 스코프**로 분류한다. 아래를 마쳐야 일반
사용자에게 열린다.

1. [Google Cloud Console](https://console.cloud.google.com/) → **API 및 서비스** →
   **라이브러리**에서 `Google Calendar API`를 **사용 설정**한다.
2. **OAuth 동의 화면** → 범위 → `.../auth/calendar.readonly`를 **추가**한다(§1d에서 만든
   그 동의 화면이다 — 클라이언트 ID도 그대로 쓴다. `VITE_GOOGLE_CLIENT_ID`는 이미 있다).
3. 동의 화면의 **승인된 자바스크립트 원본**에 `https://geurio.com`(+ 프리뷰 도메인)이
   들어 있는지 확인한다 — 토큰 요청도 이 목록을 본다.
4. **앱 검수 제출.** 검수 전에는 **테스트 사용자**(동의 화면에 등록한 계정, 100명까지)만
   쓸 수 있고 "확인되지 않은 앱" 경고가 뜬다. 검수에는 보통 앱 사용 영상과 스코프가
   필요한 이유 설명이 요구된다.
...
그 일정은 칩으로 그리지 않고 **날짜를 일요일 색으로** 칠한다(`MonthCell.holiday` — PR1부터
비워 둔 자리). 공휴일까지 칩으로 늘어놓으면 달력이 빨간 알약으로 덮인다.

### 확인

연결 뒤 `sessionStorage.mf_gcal_token`이 차고, 워크스페이스 블롭(`workspaces.data`)에
`google.calendars`가 남으며 **토큰은 남지 않는다**. 일정이 안 보이면 순서대로 본다:
① 설정 구획에 오류 문구가 있는가(스코프 미승인·테스트 사용자 아님) ② 그 캘린더가 목록에서
체크돼 있는가 ③ 콘솔 네트워크 탭에서 `calendar/v3/calendars/.../events`가 200인가.

### 하지 않은 것

- **쓰기.** 스코프 자체가 읽기 전용이다. 구글 일정은 상세 팝업에서도 고칠 수 없고
  `Google에서 열기`로 보낸다.
- **웹훅·증분 동기화**(`syncToken`·push notification) — 서버가 있어야 한다. 지금은 달을
  옮길 때마다 그 구간을 다시 받는다(캘린더 하나에 요청 하나, 한 달치).
