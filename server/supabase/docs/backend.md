...
`connected`의 뜻도 그래서 좁혀 뒀다 — **"끊겼다"는 켜져 있던 것이 꺼진 순간**(진짜
해제)과 클라이언트 ID가 없는 배포뿐이다. "지금 조회하지 않는다"(문서 위젯·닫힌 설정의
`mode: 'off'`)와 홈 하이드레이션 전(`enabled`가 아직 거짓)은 **아직 모른다**이지 끊긴
것이 아니다: 거기서 끄면 이미 연결해 둔 사람도 화면을 열 때마다 "연결 안 됨"을 한
프레임 지나 온다(연동 화면이 한순간 "연결하면 …"이라 말하던 제보의 원인).
