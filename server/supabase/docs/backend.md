...
3. **설정 → 비밀번호 변경** — 예전에는 이 화면이 없어서 비밀번호를 바꾸려면 **로그아웃하고**
   ...
   - **Google로만 가입한 계정**은 바꿀 비밀번호가 없어 항목을 비활성 + 이유를 적는다
     (판정은 `emailSignInProviders` RPC(0013) 재사용). 확인 불가(RPC 미배포·네트워크·
     데모)면 **비밀번호 있음**으로 본다 — 진짜 게이트는 현재 비밀번호 확인이므로,
     모르는 채로 항목을 잠그는 쪽이 더 나쁘다.
