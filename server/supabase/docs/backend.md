...
| 쓰기 | 읽을 수 있는 사람 — **보기 전용도 댓글은 달 수 있다** |
...
댓글은 본문이 아니므로 협업 채널(Realtime)을 타지 않는다. 상대의 새 댓글은 **패널을
열 때** 다시 읽어서 본다(`CommentStore.list`). 실시간까지 필요해지면 그때 채널을
붙이면 된다 — 지금은 왕복 하나가 더 싸다.

### 배포

`supabase/migrations/0020_document_comments.sql` — GitHub 연동으로 자동 배포된다.
