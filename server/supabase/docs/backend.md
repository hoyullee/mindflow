...
- **구간과 시각**(팝업 모양은 구글의 그 화면과 같다): 종일이면 **시작–종료 구간**이고
  (끝은 배타적인 다음 날), `시간 추가`를 켜면 **하루 안의 시각 구간**이 된다 — 구글이
  시각 근무 위치를 하루로 제한하므로 둘은 함께 쓰이지 않는다(`workLocationWhen`이 그
  규칙을 한 곳에서 정한다: 시각이 있으면 구간을 접는다).
- **고치기는 바뀐 것만**(`workLocationPatch(draft, whenChanged)`) — 갈래·이름만 바꿨으면
  `workingLocationProperties` 하나뿐이고, 구간이 바뀌면 `start`/`end` 짝이 함께 간다
  (`workLocationWhenChanged`가 판단). 구간을 싣지 않는 저장은 그 사이 구글이 스스로 판을
  올려도 412로 막히지 않는다.
- **여러 날에 걸친 근무 위치도 고친다.** 예전에는 잠갔지만 구글의 근무 위치 화면 자체가
  구간을 다루므로 잠글 근거가 없다 — 팝업이 **그 구간을 그대로 보여 주고** 고치면 구간
  전체가 바뀐다(무엇이 바뀌는지 화면이 말하므로 조용히 어그러지지 않는다).
