...
3. 브라우저: 그 해시를 **클라이언트가 읽기 전에**(엔트리 `main.tsx`) 낚아채 주소에서
   지우고, 갱신 토큰만 `geurio://auth?refresh_token=…`로 앱에 넘긴다. 브라우저에는
   **세션이 서지 않는다** — 그래서 지울 사본도, 끊을 세션도 없고, 그 사람이 웹에서
   따로 로그인해 둔 세션도 건드리지 않는다.
4. 앱: `refreshSession({ refresh_token })`으로 자기 세션을 세운다. 갱신 토큰은 쓰는
   순간 회전하므로 주소에 실려 지나간 값은 그 자리에서 무효가 된다.

### 함정 — 두 번 틀렸다 (둘 다 제보로 드러났다)
