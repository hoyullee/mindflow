...
`auth.reauthenticate()` → 계정 이메일로 6자리 nonce → `updateUser({ password, nonce })`
→ 성공하면 `signOut({ scope: 'others' })`(§15와 같은 규칙).

코드 단계를 뺄 수 없는 이유: 확인할 현재 비밀번호가 없는데 세션만으로 비밀번호를 걸게
두면 공용 PC에 남은 로그인으로 **남이 두 번째 출입구를 만들 수 있다**. "이 메일함의
주인인가"가 그 자리의 본인 확인이다.

- 대시보드 → Authentication → Emails의 **Reauthentication** 템플릿을 쓴다(기본 제공).
  이 메일이 오지 않으면 그 템플릿·SMTP를 먼저 확인한다.
