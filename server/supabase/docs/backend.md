...
1. Resend 가입 → **도메인 인증**(geurio.com의 DNS에 SPF/DKIM TXT 추가; 도메인 없이
   테스트만이면 `onboarding@resend.dev` 발신도 가능하나 프로덕션은 도메인 인증 필수).
2. Resend → **API Keys**에서 키 발급.
3. Supabase SMTP Settings에 입력:
...
4. (선택) Authentication → Rate Limits에서 이메일 발송 한도를 필요에 맞게 상향.

> Resend API 키는 **비밀값** — Supabase 대시보드에만 입력하고 저장소·커밋·클라이언트에
> 절대 넣지 않는다(`service_role`/Client Secret과 동일 취급).

### ② 확인 메일 템플릿에 6자리 코드 넣기 (앱의 OTP 입력과 일치)

대시보드 → **Authentication → Emails → "Confirm signup"** 템플릿을 편집해 `{{ .Token }}`
(6자리 코드)를 본문에 넣는다. 앱은 링크가 아니라 이 코드를 입력받으므로 필수:
...
- "인증 코드 재전송"(`다시 보내기`)은 Supabase 모드에서 `auth.resend({ type: 'signup' })`을
  실제로 호출한다(`AuthProvider.resendSignup` → `features/auth/useLoginController.ts`).
  로컬/데모 모드에선 화면의 데모 코드를 재생성만 한다(서버 없음).
- 코드 쪽 추가 작업은 없다 — 검증(`verifyOtp`)·재전송(`resendSignup`)·데모 코드 숨김은
  이미 구현돼 있고, 대시보드의 SMTP+템플릿만 맞추면 된다.
- 인증을 아예 받지 않으려면(가장 간단): Authentication → Sign In / Providers → Email →
  **"Confirm email" 토글 OFF**. 그러면 가입 즉시 세션이 생기고 앱도 이를 그대로 처리한다
  (`signUp`의 `needsVerification`이 false가 되어 verify 단계를 건너뜀).
