...
액세스 토큰은 **블롭에 오지 않는다.** 한 시간짜리이고 기기·탭마다 다르므로 **이 기기**
(localStorage `mf_gcal_token`)에 두고, 옛 탭 저장소 값은 읽을 때 한 번 이사시킨다.

**만료되면 조용히 다시 받지 않는다** — GIS 토큰 요청은 `prompt: ''`(조용한 갱신)여도
**팝업 창을 연다**(이 API에는 iframe 갱신이 없다). 그래서 토큰이 없거나 만료됐으면
`needsReauth`를 세우고 화면이 `다시 연결`로 말한다(사용자 제스처에서만 GIS를 부른다).

> **"배포하면 연동이 풀린다"는 제보의 정체**: 풀리지 않는다 — 고른 캘린더는 블롭에
> 그대로 있고 끊긴 것은 한 시간짜리 **토큰**이다. 배포가 강제 리로드를 일으켜 그 만료가
> 드러나는 것이라, `다시 연결` 툴팁·설정 부제가 "다시 연결하면 고른 캘린더가 그대로
> 이어진다"고 밝힌다. **지금은 서버가 대신 갱신한다** — 아래 「연결 유지 — refresh
> token」 참고. 그 흐름이 없는 배포(로컬·데모·함수 미배포)에서만 위 문단대로 굴러간다.
...
### 검수 기록 (2026-09-03 2차 회신)

1차 지적(§5 공유·전송·공개 · 데모 영상)을 고쳐 재제출한 뒤 온 회신. **지적 하나**:

> Your privacy policy does not specify any data protection mechanisms for sensitive data.

**§5와 다른 항목이다.** 검수는 항목별 체크리스트라 한 번에 하나씩 온다 — §5는
"누구에게 주는가"(공유·전송·공개), 이번 것은 **"어떻게 지키는가"**(전송·저장 암호화,
접근 통제, 비밀 키 관리, 안전한 삭제)다. 우리 방침에는 그 구획이 **아예 없었다**
(1~9항: 수집·목적·위탁·캘린더·공유·보유·권리·문의·변경).

→ `/privacy`에 **§6 「데이터 보호 조치」** 신설(이후 번호 한 칸씩 밀림, 7~10). 담은 것:
HTTPS(TLS) · 저장 시 암호화 · **RLS 계정 단위 접근 통제** · 첨부 파일 비공개 버킷 +
서명 URL · 비밀 키는 서버 시크릿만 · 세션 보호(전체 로그아웃·비번 변경 시 타 세션 해지) ·
최소 수집(캘린더 데이터 미보관) · 안전한 삭제(탈퇴 cascade) · 사고 대응.

**곁들여 잡은 것 — 방침이 사실과 어긋나 있었다**: §4의 접근 토큰 문장이
`sessionStorage`·"탭을 닫으면 사라집니다"였는데, 연동이 자꾸 해제된다는 제보로 토큰을
`localStorage`로 옮기면서(#PR A) **코드만 바뀌고 방침이 남았다**. 보호 수단을 적는
구획이 생기는 김에 실제 구현과 맞췄다(회귀 테스트가 `sessionStorage` 문구 부재를 고정).
**교훈: 저장 위치·수명·암호화·권한 정책을 바꾸면 §4·§6을 함께 고칠 것** — 방침이
사실과 다르면 그 자체가 반려 사유다.

절차는 1차와 같다: ① 라이브(`geurio.com/privacy`)에 반영 ② Cloud Console에서 재제출
③ **그 메일에 직접 답장**해야 재검토가 돈다. 스코프·게시 상태·브랜딩은 건드리지 않는다.
...
| `캘린더 추가` | 그 주소로 `events?maxResults=1` 한 번 → 읽히면 이름(응답의 `summary`)과 함께 블롭 `google.extra`에 적고 켠다. **구독 API는 부르지 않는다**(아래) |
...
**만들 때 우리가 보내는 것 — 참석자 규칙**(요청): 새 일정의 `attendees`에는 화면에서 초대한
사람과 회의실 말고 **나 자신**도 들어간다(`{ email: 내 주소, responseStatus: 'accepted' }`).
구글은 캘린더 화면에서 만든 일정에는 주최자 행을 자동으로 넣어 주지만 **API로 만든 일정에는
넣어 주지 않아서**, 내 응답이 없어 상세 팝업의 「참석 여부」 구획이 통째로 사라졌다.
**사람을 한 명이라도 초대한 일정에만** 넣는다 — 혼자 적어 둔 일정을 "참석자 1명짜리 회의"로
만들지 않는다. 회의실만 잡은 일정도 넣지 않는다. `organizer`는 **보내지 않는다**: 구글이
그 일정이 사는 **캘린더의 주인**으로 정하는 값이다(그래서 팀 캘린더에 만들면 주최자가 그
캘린더이고 `creator`가 나다 — 화면은 `creator`를 먼저 읽어 "일정을 만든 사람"을 말한다).

**회의실이 예약을 거절하면**(요청): 구글의 회의실 캘린더는 이중 예약되면 스스로 초대를
거절한다(`responseStatus: 'declined'`). 새 왕복도 새 스코프도 없이 이미 받아 둔 응답에서
읽어(`declinedRooms`) 만든 직후 토스트 · 상세 팝업의 경고 줄 · 달력 칩의 `⚠`로 말한다.
**알림 센터·웹 푸시로는 내보내지 않는다** — 서버가 캘린더를 모르므로 새 표와 cron Edge
Function이 필요하고, 민감 스코프를 서버에서 쓰는 순간 재검수 위험이 붙는다.
...
**사용 중인 방을 고를 때는 한 번 묻는다**(요청) — 겹쳐 잡는 것이 의도인지 확인하고,
확인을 거쳐 잡으면 배지가 `겹쳐 예약`으로 **두 사실을 함께** 말한다(`사용 중`만 남기면
내가 잡았다는 사실이, `예약됨`만 남기면 겹친 사실이 사라진다). 이미 고른 방을 빼는
클릭은 묻지 않는다.

**누가 쓰고 있는가**(요청)도 같은 조회에서 나온다 — `checkRoom`이 불리언이 아니라
`{ busy, by, title, from, to }`를 돌려주고, 행의 툴팁과 (골랐을 때) 이름 아래 한 줄이
`사용 중 · 이호율 · 디자인 리뷰 · 09:00–09:30`처럼 보여 준다(`roomBusyLabel`). 회의실
캘린더를 "한가함/바쁨만"으로 공개해 두면 제목·주최자가 없이 시각만 오고, 그때는 화면도
시각만 말한다 — **없는 것을 지어내지 않는다.**
...
물어보면 같은 답이 나온다(`calendar/googleDirectory.ts`의 `checkRoom`). 취소된 일정,
회의실이 스스로 거절한 초대, 그리고 **지금 고치고 있는 그 일정**은 제외한다(자기 자신과
부딪힌다고 말하지 않게).

왕복은 목록 전부(앞의 60개까지)를 **네 개씩 이어 달려** 묻고, 250ms 디바운스와
`이메일|구간` 캐시가 붙는다 — 수십 개를 한꺼번에 날리면 구글이 속도 제한으로 되돌려
보내 배지가 통째로 빈다.
...
`보여 줄 캘린더`는 **구독한 캘린더**(`calendarList`)만 담는다 — 구독은 구글 쪽
개인 북마크이고 그 목록에 넣는 API(`calendarList.insert`)는 **쓰기 스코프가 따로**라
동의 화면을 고치고 **검수를 다시 받아야** 한다. 통과한 다섯 스코프를 건드리지 않기로
했으므로(위 표) 목록은 **그리오 쪽에만** 더한다:

- 저장 위치는 워크스페이스 블롭의 `google.extra = [{ id, name }]`(스페이스·테마와 같은
  per-user 블롭이라 기기 간에 따라온다). `calendars`는 그대로 "지금 보여 주는 id"다.
- 확인·이름은 `events.list` **한 번**으로 얻는다(`probeCalendar`) — 그 API는 접근할 수
  있는 **어느 캘린더 id로도** 되고 응답 머리에 그 캘린더의 `summary`가 실려 온다.
- 못 읽으면 사유를 갈라 말한다(404 주소 오류 / 403 권한 없음 / 400 형식 오류).
- **구글 구독 목록과 다른 점 둘**: 색은 `calendarList`에만 있으므로 우리가 id를 씨앗으로
  정하고, `accessRole`을 알 수 없으므로 **보기 전용**으로 둔다(새 일정 목적지에 오르지
  않고 그 일정도 고칠 수 없다 — 모르는 것을 할 수 있다고 말하지 않는다).
- 나중에 사용자가 구글에서 그 캘린더를 실제로 구독하면 `calendarList` 쪽이 이기고
  우리 항목은 조용히 승격된다(색·쓰기 권한이 그때 붙는다).
- **공휴일 캘린더는 id로 표식한다**(`isHolidayCalendarId`) — 이 표식이 곧 "칩이 아니라
  날짜를 칠한다"는 규칙의 스위치이므로, 더한 항목에 세우지 않으면 그 캘린더의 항목이
  달력에 **칩으로 늘어선다**(구독한 공휴일 캘린더는 `parseCalendarList`가 같은 규칙으로
  세운다).

#### 공휴일 국가(요청)

공휴일은 구글의 **공개 캘린더**에서 온다(`ko.south_korea#holiday@…`). 그래서 국가를
고르는 일은 그 캘린더를 목록에서 **갈아 끼우는 일**이고 새 스코프도 조회도 필요하지 않다:

- 표는 `HOLIDAY_COUNTRIES`(한국·일본·미국) 하나이고, 저장은 블롭 `google.holiday`다.
  값에는 **`off`(없음)**도 있다(`HOLIDAY_OFF`) — 공휴일을 끄는 유일한 스위치다.
- 고르면 **표의 세 나라 공휴일 id를 전부** `calendars`·`extra`에서 빼고 그 나라를 넣는다
  (`isManagedHolidayId`). 구독하지 않은 국가는 `extra`로 들어간다(위 규칙 그대로 —
  `events.list`로 읽힌다). 두 나라가 함께 켜져 같은 날을 두 번 칠하는 상태를 만들 수 없다.
- **표의 세 나라 공휴일 캘린더는 `보여 줄 캘린더` 목록에 넣지 않는다**(제보: 설정에서
  국가를 고르게 해 두고 목록에서도 체크하게 되어 있어 공휴일이 두 번 표시되는 것처럼
  보였다). 스위치는 캘린더마다 하나다 — 표에 없는 공휴일 캘린더(예: 사용자가 주소로 더한
  `en.south_korea#holiday`)는 세그먼트가 관리하지 않으므로 평범한 행으로 남는다.
- 저장된 값이 없으면 **목록에 든 공휴일 캘린더**가 지금 국가를 말하고, 그것도 없으면
  `off`다 — 예전 폴백은 `kr`이라 공휴일을 안 보고 있는 사람에게 "한국"이 켜진 채로
  보이는 거짓말이었다.
- UI는 **연결된 뒤에만** 그린다(연결 전에는 고를 수 있는 척만 하게 된다).

#### 설정에서 어디에 있나(요청)

연동은 **설정 › 계정 설정 › Google 캘린더 연동**(한 겹 더)이다 — 계정 설정에는 진입
행만 있고, 보여 줄 캘린더·캘린더 추가·공휴일 국가가 그 화면 안에 있다. 조회도 그
화면이 열려 있을 때만 돈다(`mode`가 `'events'`). 그래서 진입 행의 부제는 **왕복 없이
되는 값**(블롭의 `enabled`·`calendars`)으로만 만든다: `connected`는 목록이 도착해야
참이 되므로 그것으로 문구를 고르면 연동해 둔 사람에게 "연결하면 …"이라 말하게 된다.

`connected`는 그래서 **파생값**이다 — `enabled && !needsReauth`, 즉 "블롭이 켜져 있고
끊겼다고 알려진 바 없다". **"이 탭이 지금 액세스 토큰을 들고 있는가"가 아니다**: 그
토큰은 한 시간마다 만료되고 refresh token은 서버에만 있으므로, 토큰의 유무로 화면을
고르면 하루 뒤에 돌아온 사용자에게는 연결해 둔 것이 없는 것처럼 보인다(제보: 연동
화면이 짧게 떴다가 목록이 오며 커졌다 — 실측 구획 95px, 팝업 카드 570 → 220px).

목록의 로딩 판정도 같은 결이다 — **"목록이 비어 있는가"로 보지 않는다**: 화면이 쓰는
목록은 구독 목록 ∪ **주소로 더한 캘린더**(블롭에 있어 조회 없이 즉시 나온다)라, 더해 둔
것이 하나라도 있으면 "이미 왔다"로 오판해 스켈레톤을 건너뛴다(제보: 그 한 줄 높이로
팝업이 줄었다가 목록이 오며 커졌다). 훅이 `listLoaded`로 **구독 목록이 도착했는지를
직접** 들고, 화면은 그 값만 본다.

그래서 "끊겼다"고 말할 수 있는 것은 둘뿐이다: **갱신이 실패했다**(`needsReauth` — 구글
에서 권한을 회수했거나 서버 흐름이 없는 환경) 또는 **사용자가 껐다**(`enabled` 거짓).
"지금 조회하지 않는다"(문서 위젯·닫힌 설정의 `mode: 'off'`)와 홈 하이드레이션 전은
**아직 모른다**이지 끊긴 것이 아니다.
...
구글은 재택·사무실 표시를 **`eventType: 'workingLocation'`인 종일 일정**으로 든다. 읽는
쪽은 PR5부터 있었고(칩이 아니라 **날짜 칸 우측 상단의 한 마디** — 그 날의 상태이므로)
쓰는 쪽을 이번에 붙였다. **새 스코프는 없다** — 이미 승인된 `calendar.events`로 쓴다.

- **기본 캘린더에만.** 구글이 이 일정을 다른 캘린더에 받지 않는다. 그래서 진입점은 기본
  캘린더가 **쓸 수 있고 지금 보고 있을 때만** 뜬다(꺼 둔 캘린더에 쓰면 고른 값이 화면에
  나타나지 않아 고장으로 읽힌다).
- **본문**: `eventType` + `start`/`end` + `workingLocationProperties`(`homeOffice` /
  `officeLocation.label` / `customLocation.label`) + `visibility: 'public'` ·
  `transparency: 'transparent'`(그 날의 상태라 바쁨으로 잡히면 회의실·한가함 조회에
  걸린다). 제목은 **지어 넣지 않는다** — 구글의 클라이언트가 갈래로 보여 준다.
- **일정 하나는 언제나 하루다.** 구글이 못박아 뒀다(라이브 제보의 400):
  `malformedWorkingLocationEvent` — *"An all-day working location event must be exactly
  one day long."* 그래서 `WorkLocationDraft.endDate`는 **한 일정의 길이가 아니라 며칠에
  걸 것인가**를 뜻하고, 그 해석은 `workLocationDays` **한 곳**에서만 한다(`workLocationWhen`은
  그 값을 아예 보지 않아 여러 날이 한 일정에 실리는 상태를 만들 수 없다).
- **구간은 하루하루에 하나씩**(`useGoogleCalendar.saveWorkLocation`): 고른 구간의 날마다
  있으면 PATCH, 없으면 POST를 차례로 보내고(한꺼번에 날리면 속도 제한) 마지막에 한 번
  새로 읽는다. 상한 `WORK_LOCATION_MAX_DAYS`(31) — 넘으면 팝업이 **막고 이유를 말한다**
  (조용히 잘라 저장하면 고른 것과 저장된 것이 달라진다).
- **`시간 추가`**를 켜면 그 하루 안의 시각 구간이 된다(구글도 시각 근무 위치를 하루로
  제한한다) — 구간과 함께 쓰이지 않는다.
- **고치기는 바뀐 것만**(`workLocationPatch(draft, whenChanged)`) — 갈래·이름만 바꿨으면
  `workingLocationProperties` 하나뿐이다(`workLocationWhenChanged`가 판단). 날짜를 싣지
  않는 저장은 그 사이 구글이 스스로 판을 올려도 412로 막히지 않는다.
- **팝업이 보여 주는 것은 그 하루다.** 데이터가 실제로 하루 단위이므로(위 제약) 걸려 있는
  날을 열면 그 날이 뜨고, 종료 날짜를 뒤로 밀면 **그만큼 날이 더 걸린다** — 구간 밖의
  날은 건드리지 않는다(지우기도 그 하루만). 반복되는 근무 위치는 회차가 하루짜리로 오므로
  그 회차의 갈래·이름을 고치는 것은 그대로 된다.
- **매주 되풀이는 하루짜리에만 뜬다**(요청 — 구글의 `근무 위치 수정` 화면과 같은 두
  선택: `선택한 날짜만` / `…부터 매주`). 저장은 `repeat: 'weekly'` → **일정 하나 +
  `RRULE:FREQ=WEEKLY;BYDAY=<요일>`**이고 회차는 구글이 펼친다(`workLocationDays`가 그때
  하루만 돌려주므로 구간처럼 여러 날에 나뉘지 않는다). 요일을 규칙에 **명시**하는 이유:
  나중에 시작일만 옮기는 PATCH에서 `DTSTART`가 정하는 요일과 어긋날 수 있다.
  - **구간에서는 되풀이 칸이 사라진다** — 하루씩 여러 일정으로 나가는 것에 되풀이를
    얹으면 사용자가 고른 적 없는 모양이 된다.
  - **이미 매주 반복인 근무 위치(회차)를 열면** `매주`가 켜진 채로 뜨고, 그때 두 선택은
    **바꾸는 범위**가 된다(구글의 그 화면과 같은 뜻):
    - `매주` 유지 → 회차가 아니라 **그 반복 자체**(`recurringEventId`가 가리키는 원본)를
      PATCH한다. 규칙은 이미 그 일정에 있으므로 싣지 않는다.
    - `선택한 날짜만` → 그 **회차를 DELETE**하고 그 날짜에 홀로 선 근무 위치를 POST한다
      (데이터로도 그게 "그 날만"이다).
  - **회차를 PATCH하지 않는다.** 라이브에서 그 요청이 400
    `malformedWorkingLocationEvent`("modify a working location event in a way that is not
    valid for this event type")로 거절됐다 — 구글은 근무 위치 회차의 수정을 막는다.
    그러니 거절되는 요청을 아예 만들지 않는다(저장 규칙은 `saveWorkLocation`).
  - 반복 규칙 자체를 **캘린더 설정에서** 정하는 길(구글의 그 화면)은 우리 스코프로 닿지
    않는다 — 그건 여전히 구글 캘린더의 일이다.
- **우리 표에는 없는 개념이다**(§18의 `calendar_events`) — 구글 전용이다.

구글이 거절하면(400) 사유가 그대로 화면·콘솔에 남는다(아래 `PATCH는 바뀐 것만` 참고).
Workspace 정책이나 계정 종류에 따라 이 기능이 꺼져 있을 수 있으므로, 실패하면 그 문장이
유일한 단서다.

### 일정 색은 구글이 정하는 만큼(요청)

값이 `colorId`이므로 **고를 수 있는 색은 구글이 정한다** — 표에 없는 번호를 우리가 지어
내면 그 저장이 거절된다. 그래서 `googleColorOptions`는 목록을 적어 두지 않고 `/colors`가
돌려준 번호를 **번호순으로 전부** 싣는다(응답 순서에는 보장이 없다). 구글이 팔레트를
넓히면 화면도 그만큼 넓어진다(24개를 주면 24칸). 이름은 아는 열한 개는 구글의 낱말
(라벤더·세이지·토마토…), 모르는 번호는 hex에서 계산한다 — 글자 없는 동그라미에 접근
이름·툴팁이 없으면 무슨 색인지 알 길이 없다. 팔레트를 못 받으면 폴백 표
(`GOOGLE_EVENT_COLORS`)로 그리고 콘솔에 왜 못 받았는지 남긴다.

**몇 색을 받았는지도 콘솔에 남긴다** — `[geurio] 구글 이벤트 색 N개(번호 …)`. "구글 웹
UI에는 24색인데 우리 팝업은 11칸"의 답이 그 한 줄에 있다: 이 API가 11개를 주면 나머지는
...
### 설치형 앱의 연동 — 시스템 브라우저 + 딥링크 (제보)

앱에서 `연결하기`를 누르면 **그 자리에서** `팝업이 막혔어요`가 떴고, 브라우저에서 동의를
마쳐도 앱으로 이어지지 않았다. 원인은 하나다 — 웹의 GIS 흐름은 `window.open`으로 동의
창을 띄우는데 **앱 창은 우리 출처만 띄우므로**(셸의 `setWindowOpenHandler`) 그 요청이
시스템 브라우저로 넘어가고, GIS에게는 `window.open`이 `null`을 돌려준 것으로 보인다
(`popup_failed_to_open`). 넘어간 창은 `opener`가 없어 코드를 돌려줄 곳도 없다.

그래서 설치형 앱은 **GIS를 아예 지나지 않고** 로그인과 같은 길(RFC 8252, §21)을 쓴다:

1. 앱이 동의 주소를 만들어 **시스템 브라우저**에서 연다(`response_type=code`,
   `access_type=offline`, `prompt=consent` — 이 둘이 refresh token을 받는 짝이다).
2. 브라우저: 동의 → `https://geurio.com/auth/gcal?code=…&state=…`
3. 그 페이지가 `geurio://gcal?code=…&state=…`로 앱을 깨운다.
4. 앱이 `state`를 대조하고 코드를 **Edge Function `google-oauth`**에 넘겨 교환한다
   (client secret은 서버에만 있다).

**스코프는 하나도 늘지 않는다** — 검수를 통과한 다섯 그대로다.

> **소유자가 해야 할 콘솔 작업 한 줄**: Google Cloud Console → API 및 서비스 → 사용자
> 인증 정보 → 그 **웹 클라이언트** → **승인된 리디렉션 URI**에
> `https://geurio.com/auth/gcal` 추가. 리디렉션 URI는 **검수 대상이 아니므로** 스코프·게시
> 상태·브랜딩은 건드리지 않는다. 기존 URI(Supabase 콜백)는 **지우지 말 것** — 웹 로그인이
> 계속 쓴다. 이 줄이 없으면 브라우저에서 구글의 `redirect_uri_mismatch` 오류 화면이 뜬다.

교환 요청의 `redirect_uri`는 **코드를 받을 때 쓴 값 그대로**여야 한다(구글의 규칙) — 웹의
GIS 팝업은 약속값 `postmessage`, 앱은 위 핸드오프 주소다. 함수는 그 값을 받되
`postmessage`이거나 `https://…/auth/gcal`인지 **모양을 확인한다**(우리 시크릿을 쓰는
자리라 좁혀 둔다).

서버 흐름이 없는 배포(로컬·데모·함수 미배포)에서는 코드를 받아 와도 교환할 곳이 없으므로
**창을 열지 않고** 그 사실을 문장으로 말한다.

### "다시 연결"은 탭 전체가 함께 안다 (제보)

설정은 `다시 연결`이라 하는데 **일정 화면·대시보드 위젯·새 일정 팝업은 연결된 것처럼**
보였다. `needsReauth`가 `useGoogleCalendar` **인스턴스마다** 따로 있었기 때문이다 — 연결
상태는 계정 하나의 사실이라 화면마다 다를 수 없다. 지금은 토큰과 같은 자리
(`googleCalendar.ts`)에 두고 바뀌면 모두에게 알린다.

그리고 그 상태에서는 **구글이 화면에서 빠진다**: 이 탭이 기억한 일정(`eventCache`)은
그리지 않고(공휴일 칠·근무 위치도 같은 목록에서 오므로 함께), 새 일정 팝업의 "저장할
캘린더"에서 구글 목적지가 사라진다(골라 봐야 저장이 실패한다). 다시 연결하면 곧바로
돌아온다.
...
구글은 두 경우에 **참석자 배열을 온전히 주지 않는다**: 참석자가 아주 많으면
`attendeesOmitted: true`로 잘라 보내고, 주최자가 "참석자 명단 보기"를 껐으면
(`guestsCanSeeOtherGuests: false`) 나 자신만 담아 준다. 이 상태에서 우리가 들고 있는
배열을 **되돌려 쓰면 나머지 참석자가 조용히 초대 취소된다** — PATCH는 `attendees`를 통째로
바꾸기 때문이다(§19 「쓰기 규칙」).

그래서 `attendeesLocked()`가 그 두 경우를 잡아 ① 화면에 안내를 띄우고(참석자·참여 여부
칸을 잠근다) ② **쓰기 경로에서 `attendees`를 아예 싣지 않는다**(제목만 고치면 PATCH의
키가 `summary` 하나다). 명단을 고치려면 Google 캘린더에서 한다. 주최자 본인이 명단 보기를
껐을 뿐인 경우(`organizer.self`)는 배열이 온전하므로 잠그지 않는다.
...
## 22. 일정 알림 (0038 `calendar_events.reminder_minutes`) — 서버는 보내지 않는다

요청: *"오전 10:30 일정에 10분 알림을 걸어 두면 10:20에 앱 알림과 OS 알림을 받고 싶다."*

### 마이그레이션

`0038_event_reminders.sql` — `calendar_events`에 `reminder_minutes int` 추가(+ 0~40320
범위 check). `null` = 알림 없음이고 **그게 기본**이다: 아무것도 저절로 뜨지 않는다.
...
미적용 서버에서도 앱은 깨지지 않는다 — 어댑터가 그 칼럼을 뺀 행으로 **한 번 더** 저장하고
콘솔에 `반복·알림 없이 저장했어요(마이그레이션 0034·0038 확인)`를 남긴다(배포 순서 안전).

### 왜 서버 푸시가 아닌가 (이 결정이 설계의 뿌리다)

이 칼럼은 **누가 언제 알림을 원하는가**만 담고, 띄우는 일은 브라우저·앱이 한다
(`apps/web/src/features/reminders/`). 서버가 알림을 보내려면 일정의 제목·시각을 서버에
쌓아야 하는데, 개인정보처리방침이 캘린더 데이터를 *"브라우저에서 화면에 그리는 데만 쓰고
서버·수탁사를 거치지도 저장되지도 않는다"*고 **명시**하고 있고(§4·§5·§6) **구글 민감
스코프 검수가 그 방침으로 통과했다**. 로컬 예약이면 방침·스코프를 한 줄도 건드리지 않고
"앱이 켜져 있을 때"를 온전히 덮는다.

| 상황 | 지금 | 왜 |
| --- | --- | --- |
| 앱·탭이 열려 있다(최소화 포함) | ✅ | 30초 주기 확인 + OS 알림 |
| 데스크톱 앱 창을 닫음 | ✅ | 트레이(macOS는 독)에 남아 렌더러가 계속 돈다 — 아래 §22d |
| 데스크톱 앱 완전 종료 | ❌ | 그때는 멎는다 — 사용자가 고른 것이다 |
| 브라우저 닫힘 | ❌ | 서버 푸시가 필요 |
| 모바일 앱 (닫혀 있어도) | ✅ | Capacitor 로컬 알림 — **OS가 예약을 들고** 있어 서버 없이 된다. 아래 §22c |
| 구글 일정 | ✅(기본 꺼짐) | 구글이 이미 보내므로 켜면 둘이 된다 — 아래 §22b |

Geurio 일정에 한해서는 서버 푸시가 **방침 개정 없이** 가능하다(그 값이 원래 우리 표에
있다) — "앱을 완전히 닫아도 알림"이 필요해질 때의 다음 수순이고, 그때도 구글 일정은
올리지 않는다.

### 왜 `setTimeout` 하나가 아니라 주기 확인인가

① 백그라운드 탭의 타이머는 크롬이 1분에 한 번으로 조인다 ② **절전 중에는 타이머가 아예
돌지 않는다**. 그래서 30초마다 "지금 띄울 것"을 훑고, 지나간 알림은 **5분 유예** 안이면
늦게라도 띄우고 그보다 오래면 조용히 넘긴다(어제 알림이 오늘 앱을 켜자마자 뜨지 않게).
설치형 앱은 `backgroundThrottling: false`로 그 조임 자체를 없앤다(셸만 정할 수 있다).

### 종일 일정

v1은 **시각 있는 일정만** 알림을 갖는다 — 종일에 "10분 전"은 자정 10분 전이라 뜻이
어긋나고(구글도 종일 알림을 "며칠 전 몇 시"라는 다른 표기로 다룬다) 우리 칩은 분 단위뿐이다.
종일로 바꾸면 정규화가 값을 **지운다**(저장본만 봐도 무엇이 뜰지 알아야 한다).

### 진단

| 증상 | 볼 곳 |
| --- | --- |
| 알림이 안 뜬다 | 설정 › `일정 알림`이 켜져 있는가 · 그 행의 문구가 권한 상태를 말한다 |
| 앱 알림만 뜬다 | OS 알림 권한이 `denied` — 브라우저·OS 설정에서 풀어야 한다(앱에서 못 푼다) |
| 저장이 안 된다 | 콘솔 `반복·알림 없이 저장했어요` → 0038 미적용 |
| 같은 알림이 두 번 | `localStorage.mf_reminded`(키 = `일정#회차@알림시각`) 확인 |

### 22b. 구글 일정 알림 (2단계) — 마이그레이션 없음

**서버는 한 줄도 바뀌지 않는다.** 구글 일정의 알림 값은 구글이 들고 있고, 우리는 이미
읽고 있다(`reminders.useDefault` / `overrides`). 2단계가 더한 것은 **띄우는 쪽**뿐이다.

**기본이 꺼짐**인 이유는 1단계와 정반대다: 그 알림은 **구글이 이미 보낸다**(구글 캘린더
앱·브라우저 알림). 켜진 채로 시작하면 같은 회의에 알림이 둘 뜨는 것이 기본 동작이 되고
그건 고장으로 읽힌다. 설정 › `일정 알림` 아래 하위 토글이고(`mf_reminders_google`,
이 기기) **연동돼 있을 때만** 그린다.

#### 몇 분 전에 띄울까 — 두 곳에서 온다

| 일정의 `reminders` | 우리가 쓰는 값 |
| --- | --- |
| `overrides: [{popup, 10}]` | 10분 — 그 일정에 직접 건 알림 |
| `useDefault: true` | **그 캘린더의 기본 알림**(`calendarList`의 `defaultReminders` 첫 팝업) |
| `useDefault: false`, overrides 없음 | 없음 — 사용자가 꺼 둔 것이다 |
| 기본을 모르는 캘린더 | 없음 — 모르는 값을 지어내지 않는다 |

대부분의 구글 일정이 `useDefault`라, 캘린더 기본을 모르면 이 기능이 사실상 비어 버린다.
그 값은 **`calendarList`에만 있고 일정 응답에는 없으므로** 목록을 받을 때 함께 챙긴다
(`GoogleCalendarMeta.defaultMinutes`).

#### 캘린더 거울 (`mf_gcal_remind_cals`)

정본은 워크스페이스 블롭(`google.calendars`)이고 그것을 읽는 것은 홈이다. 그런데 알림
스케줄러는 문지기(`RequireAuth`) 안에 있어 **에디터에서도** 도는데 에디터는 그 블롭을
한 번도 읽지 않는다 → 홈이 알고 있을 때 이 기기에 적어 둔다(`storeReminderCalendars`).

```js
// 브라우저 콘솔
JSON.parse(localStorage.getItem('mf_gcal_remind_cals'))
// → [{ id: 'me@example.com', defaultMinutes: 10 }]
```

담는 것은 **지금 보여 주는 캘린더 + 그 기본 알림**뿐이다: 숨긴 캘린더의 일정이 알림으로
뜨면 화면과 어긋나고, 공휴일 캘린더는 종일이라 담지 않는다. 목록이 **도착하기 전에는
적지 않는다**(그때는 주소로 더한 것뿐이라 구독 캘린더를 통째로 지운 거울이 된다).

#### 빼는 것 넷

종일(자정 기준이라 뜻이 어긋난다) · 공휴일 · 근무 위치(회의가 아니다) · **내가 거절한
회의**(구글도 알리지 않는다).

#### 조회 비용

켜 둔 사람에게만, 알림 창(오늘~+2일)에 대해 **캘린더 수만큼** 요청이 5분 주기 + 깨어나는
순간에 난다. 끈 사람에게는 **왕복이 한 번도 나가지 않는다**(테스트로 고정). 토큰은
`ensureGoogleToken`으로 **팝업 없이** 받고, 못 받으면 조용히 물러난다 — 알림 때문에 동의
창이 저절로 뜨면 안 되고(§19 재로그인 팝업), "다시 연결"을 세우는 것은 화면의 일이다.

#### 진단

| 증상 | 볼 곳 |
| --- | --- |
| 구글 하위 토글이 없다 | 연동돼 있는가(설정 › 계정 설정 › Google 캘린더 연동) · 위 `일정 알림`이 켜져 있는가 |
| 켰는데 안 뜬다 | `mf_gcal_remind_cals`가 비어 있지 않은가(홈·일정 화면을 한 번 열면 채워진다) |
| 특정 일정만 안 뜬다 | 그 일정이 `useDefault`인데 캘린더 기본이 없을 수 있다 — 구글에서 그 캘린더의 기본 알림 확인 |
| 알림이 둘 뜬다 | 정상 — 구글도 보내고 우리도 보낸다. 이 토글을 끄면 구글 것만 남는다 |

### 22c. 모바일 로컬 알림 (3단계) — 마이그레이션 없음

1·2단계는 **앱이 켜져 있을 때**만 동작한다(우리가 30초마다 훑는다). Capacitor 앱에서는
**OS가 예약을 들고 있어** 앱이 닫혀 있어도, 기기를 재부팅해도 그 시각에 뜬다
(`@capacitor/local-notifications`, 플러그인이 `BOOT_COMPLETED`에 다시 심는다).

**서버는 한 줄도 바뀌지 않는다** — 클라이언트가 앱이 열려 있는 동안 앞으로 며칠치를
OS에 옮겨 둘 뿐이다. 개인정보처리방침의 "캘린더 데이터는 브라우저에서만 쓰고 서버에
저장하지 않는다"(§4·§5·§6)도 그대로다: 예약은 기기 안에서 끝난다.

#### 무엇을 맡기는가

| 값 | 근거 |
| --- | --- |
| 창 7일(`NATIVE_WINDOW_DAYS`) | 웹은 2일이면 되지만(켜져 있을 때만 돈다) 여기서는 주말 내내 앱을 열지 않은 사람의 월요일 아침이 덮여야 한다 |
| 최대 48건(`NATIVE_MAX_PENDING`) | iOS는 앱당 64건까지만 들고 있고 **넘치면 조용히 버린다** — 여유를 두고 가까운 것부터 채운다 |
| 지난 알림은 제외 | OS 예약은 미래만 뜻이 있다. 지나간 알림을 늦게라도 띄우는 일은 앱이 켜져 있을 때의 5분 유예가 맡는다 |
| id = `해시(key@fireAt\|title)` | 일정을 옮기거나 제목을 고치면 **다른 알림**이 되어 옛 예약이 거둬진다. 회차는 키가 이미 구분한다 |

동기화는 **집합 차이**다(`planNativeSchedule`): 걸려 있는 id와 있어야 할 id를 견줘 빠진
것만 심고 남는 것만 거둔다. 통째로 지웠다 다시 심으면 곧 뜰 알림이 그 찰나에 사라진다.
알림을 끄면 목록이 비어 **같은 경로로** 전부 거둬진다(끄는 길을 따로 만들지 않는다).

#### 누가 띄우는가 — 둘이 되지 않게

권한이 있으면 OS가 띄우고 **우리 주기 확인은 판단에서 빠진다**. 앱이 떠 있는 동안의
토스트는 플러그인의 수신 이벤트(`localNotificationReceived`)로 잇는다(iOS는 그때 배너를
감추므로 그것이 유일한 표시다). 권한이 없으면 맡길 수 없으므로 **예전처럼 우리가**
훑어 띄운다. 권한을 아직 모르는 동안(앱을 켠 직후)에는 **아무도 띄우지 않는다** —
모르는 채로 우리가 먼저 띄우면 곧이어 OS도 띄운다.

#### 함정 셋(코드 주석과 같은 내용)

- **정확 알람을 쓰지 않는다**(`isExactNotification: false`). 안드로이드 12+에서 정확
  알람은 따로 허용받아야 하고, 플러그인은 허용돼 있지 않으면 `schedule()` 도중
  **시스템 "알람 및 리마인더" 설정 화면을 연다** — 앱을 열면 저절로 도는 동기화가
  누르지도 않은 시스템 화면을 띄우면 안 된다(§19 재로그인 팝업과 같은 규칙). 대가는
  몇 분의 오차이고 `allowWhileIdle: true`로 Doze는 넘긴다.
- **알림 아이콘은 흰 글리프 + 투명 배경**(`ic_stat_geurio`). 안드로이드는 색을 버리고
  알파만 남기므로 런처 아이콘을 쓰면 상태 표시줄에 흰 네모가 뜬다.
- **플러그인 객체를 async 함수에서 그대로 `return`하지 않는다.** 반환 처리가 thenable
  인지 보려고 `.then`을 읽는데 Capacitor 플러그인은 모르는 이름을 전부 네이티브 호출로
  넘기는 프록시라 그 순간 거부된다 — 상자에 담아(`{ p }`) 돌려준다.

#### 진단

| 증상 | 볼 곳 |
| --- | --- |
| 설정에 `일정 알림` 행이 없다 | 네이티브에서는 웹 `Notification`이 없으므로 플러그인 권한으로 판단한다 — 플러그인이 네이티브에 등록됐는지(`android/app/capacitor.build.gradle`에 `capacitor-local-notifications`) |
| 앱을 닫으면 안 온다 | 권한이 `granted`인가 · 그 알림이 창(7일) 안이고 미래인가 · 48건 상한에 밀리지 않았는가 |
| 몇 분 늦게 온다 | 정상 — 위의 정확 알람 트레이드오프 |
| 상태 표시줄 아이콘이 흰 네모 | `capacitor.config.ts`의 `LocalNotifications.smallIcon`과 `res/drawable-*/ic_stat_geurio.png` |
| 알림이 둘 뜬다 | 권한 판정이 늦어 우리도 띄운 경우 — 앱 로그에서 `nativeOwned`가 `true`로 서는지 |

### 22d. 데스크톱 트레이 상주 (4단계) — 마이그레이션 없음

설치형 앱에서 **창을 닫아도** 알림이 계속 오게 한다. 알림을 띄우는 것은 렌더러의 주기
확인이므로, 창을 파괴하면 그 순간 스케줄러도 사라진다 — 그래서 닫기를 **숨기기**로 바꿔
창(=렌더러)을 살려 둔다. 모바일(§22c)이 OS에 예약을 **맡기는** 것과 달리, 여기서는
**우리가 계속 돌아** 3단계의 장치(주기 확인·유예·중복 방지)를 그대로 쓴다.

**서버도 웹 번들의 알림 규칙도 바뀌지 않는다** — 셸이 창을 살려 두고, 웹은 설정 두 행과
"알림을 누르면 창을 되찾는 길"만 더한다.

#### 규칙(셸의 순수 함수가 정한다 — `apps/desktop/src/shell.ts`)

| 판단 | 규칙 | 왜 |
| --- | --- | --- |
| 숨길까(`canStayInBackground`) | macOS는 언제나, 그 밖은 **트레이가 실제로 만들어졌을 때만** | 숨겼는데 트레이도 독도 없으면 창을 되찾을 길이 사라진다 |
| 트레이를 둘까(`usesTray`) | macOS 제외 | 독 아이콘이 이미 "실행 중"이자 되돌아오는 길이다(그 플랫폼 관례) |
| 창 없이 시작할까(`shouldStartHidden`) | 로그인 자동 실행(`--hidden`·`wasOpenedAtLogin`)이고 **상주할 수 있을 때만** | 닿을 길 없는 프로세스를 만들지 않는다 |
| 로그인 자동 실행을 내줄까(`supportsOpenAtLogin`) | Windows·macOS | Electron의 `setLoginItemSettings`가 Linux를 무시한다 |

기본은 **켜짐**이고(Slack·Teams·Discord와 같다), 대신 **처음 숨길 때 한 번 알린다** —
그 안내는 **실제로 띄운 뒤에만** "알렸다"로 기록한다(못 띄운 환경이면 다음에 다시 알린다).

#### 상태의 정본은 셸이다

상주 여부는 `userData/settings.json`, 로그인 자동 실행은 **OS**(`getLoginItemSettings`)가
들고 있다. 렌더러는 사본을 만들지 않고 `backgroundState()`가 돌려주는 것을 그대로 그리며,
토글도 **바뀐 뒤의 상태**를 돌려받는다 — MSIX 컨테이너처럼 로그인 항목을 받아들이지 않는
환경에서는 스위치가 제자리로 돌아가 사실을 말한다(거짓말을 하지 않는다).

`supported`가 거짓이면 설정에 **그 자리를 그리지 않는다**: 브라우저·PWA, 4단계 이전
설치본(창구 자체가 없다), 트레이가 없는 리눅스 데스크톱.

#### 함정 넷

- **AUMID**(`app.setAppUserModelId`)가 `electron-builder.yml`의 `appId`와 달라지면
  Windows 토스트가 아예 뜨지 않거나 남의 이름으로 뜬다. 알림이 이 단계의 전부라 그 한
  줄이 기능의 전제다 — `packaging.test.ts`가 두 값을 묶어 둔다.
- **트레이 아이콘은 `resources/`에 둔다.** `build/`는 electron-builder가 쓰는 재료일 뿐
  앱에 담기지 않으므로, 거기 두면 개발 실행에서는 보이고 **설치본에서만** 사라져 상주가
  통째로 없어진다.
- **트레이 아이콘은 설정과 무관하게 늘 띄운다.** 끈 상태에서 아이콘까지 없애면 상주가
  "불가능한 환경"으로 읽혀 설정의 그 자리가 사라지고 다시 켤 길이 없어진다.
- **`before-quit`에서 종료 깃발을 세운다.** 닫기를 가로채는 규칙이 종료까지 막으면 앱을
  끌 수 없다.
...
| 증상 | 볼 곳 |
| --- | --- |
| 설정에 `앱을 닫아도 알림 받기`가 없다 | 셸 판이 4단계 이전이거나(`window.geurio.backgroundState`가 없다) 트레이를 만들지 못한 환경 |
| 닫으면 그냥 꺼진다 | 그 설정이 꺼져 있는가(`userData/settings.json`의 `background`) |
| 닫아 두면 알림이 안 온다 | 창이 **숨은 것인지 파괴된 것인지** — 트레이 `Geurio 열기`로 되살아나면 숨은 것이다 |
...
## 24c. 설치형 앱의 멘션 배너 — 푸시 없이 같은 일을 한다

### 왜 필요했나

푸시(§24)는 **설치형 셸에서 쓸 수 없다**. 그래서 앱을 닫아 둔 데스크톱 사용자에게는
**30분 뒤 메일이 유일한 통로**였다. 사용자가 그 자리를 채워 달라고 했다.

### 왜 새 설치본이 필요 없나 (중요)

이 셸은 **닫기 = 숨기기**다(트레이 상주, §22 계열). 창이 숨어도 **렌더러는 살아 있고**
알림 실시간 구독(`NotificationsContext`의 `store.subscribe`)도 계속 돈다 — 작업 표시줄
배지가 이미 그렇게 움직인다(`DesktopBadgeHost`). 즉 "새 멘션이 왔다"는 사실은 이미
알고 있었고 **배너로 띄우지만 않았다**. 띄우는 창구도 이미 있다(`geurio:notify` —
일정 알림이 쓴다).

그래서 이 기능은 **preload 창구를 새로 뚫지 않는다** → CLAUDE.md의 "새 창구를 뚫으면
반드시 새 설치본" 규칙에 걸리지 않고, **이미 깔린 설치본에도 웹 배포만으로 닿는다**.

### 규칙 (푸시·메일과 나란히 둔 근거)

| | 값 | 근거 |
| --- | --- | --- |
| 종류 | `mention`·`doc_mention` | 메일·푸시와 같은 범위(`KINDS` 한 줄로 넓힌다) |
| 나이 컷 | **1시간** | 푸시와 같다 — "한 시간 전 일"을 배너로 띄우는 건 소음 |
| 언제 | **사용자가 보고 있지 않을 때만** — 창이 보이면서 **포커스까지** 있을 때만 조용하다 | 보고 있으면 벨 배지가 이미 말한다. 이 판단을 `visibilityState`로만 하면 **설치형 앱에서 영영 안 뜬다** — 아래 ⚠️ |
| 설정 | `notification_prefs.push_mentions` **공유** | 사용자에게 둘은 "앱을 안 보고 있을 때 알려 줄까" 하나이고 기기마다 길이 다를 뿐 |
| 본문 | 싣지 않는다 | 잠금 화면에 남의 문장이 뜨지 않게(푸시와 같은 판단) |

설정 화면에는 **셸 전용 행**(`data-mention-shell-row`, "멘션 알림")이 푸시 행 자리에
선다. 이게 없으면 설치형 앱 사용자는 **자기에게 뜨는 배너를 끌 길이 없다**(푸시 행은
그 셸에서 그려지지 않는다).

### 되풀이를 막는 두 겹 (이 기능의 진짜 위험)

"안 뜬다"보다 **"너무 뜬다"**가 위험하다 — 목록은 여러 계기로 다시 읽히고(실시간
신호·60초 주기·탭 복귀) 앱은 껐다 켜진다.

1. **기억**(`mf_mention_announced`, localStorage) — 이미 알린 id. 앱을 껐다 켜도 남고,
   창이 여럿일 때를 위해 **띄우기 바로 앞에서 저장소를 새로 읽는다**(`mf_reminded`와
   같은 판단).
2. **기준선** — 첫 조회 결과는 전부 "이미 안 것"으로 적고 끝낸다. 안 그러면 앱을 켤
   때마다 밀린 멘션이 우르르 뜬다.

⚠️ **②에서 한 번 깨졌다**: 마운트 직후 한 프레임은 `items`가 **언제나 빈 배열**이라,
그걸 기준선으로 잡으면 곧이어 도착한 진짜 목록이 통째로 "새 알림"이 된다. 그래서
`NotificationsContext`에 **`loaded`**(첫 조회가 끝났는가)를 노출하고 그 전에는 아무
판단도 하지 않는다. 조회가 **실패해도** `loaded`를 세운다 — 안 그러면 서버가 조용할 때
배너가 영영 기준선을 못 잡는다.

그리고 **보는 동안 도착한 것도 "본 것"으로 적는다** — 안 그러면 나중에 창을 숨기는
순간 묵은 멘션이 뒤늦게 튀어나온다.

### ⚠️ 첫 판은 한 번도 뜨지 않았다 — `visibilityState`를 믿었기 때문 (제보, 2026-09-16)

첫 판은 "보고 있는가"를 `document.visibilityState === 'visible'` 하나로 쟀다. **설치형
앱에서는 그 값이 언제나 `visible`이다.** 셸이 창을 `backgroundThrottling: false`로
만들기 때문이다(`apps/desktop/src/main.ts`) — 창을 내려 둬도 **일정 알림 타이머가
돌아야 해서 일부러 끈 값**이다. Electron 문서가 그 대가를 그대로 적어 두었다:
...
> — *electron/docs/api/browser-window.md, "Page visibility"*

그래서 **창을 트레이에 닫아 둔 바로 그 상태에서 `visible`이 돌아오고**, 가드가 언제나
먼저 걸려 배너가 한 번도 뜨지 못했다. 게다가 그 앞에서 `mf_mention_announced`에 이미
적은 뒤라 **나중에도 안 뜬다**.

**지금은 `isUserWatching()`이 둘을 함께 본다: 보이면서 포커스까지 있을 때만 조용하다.**
창이 숨거나 최소화되면 포커스는 잃고, 그 신호는 셸의 그 설정과 **무관하다**. 다른 앱에
가려 뒤에 있는 창은 알리는 쪽으로 기운다 — 규칙의 목적이 "보는 사람에게 겹쳐 띄우지
않는다"이지 "떠 있기만 하면 알리지 않는다"가 아니다.

**왜 테스트가 못 잡았나**: jsdom은 우리가 `hidden`이라고 하면 `hidden`이라고 답한다 —
**실기기가 만들 수 없는 상태를 심어 놓고 통과한 것**이다. 그래서 테스트의 기본 상태를
셸이 실제로 답하는 값(`visible`인데 포커스는 없다)으로 바꿨고, 그 자리를 못박는 테스트를
두 건 더했다(옛 가드로 되돌리면 4건이 깨지는 것을 확인했다).

**곁가지로 드러난 것 둘**(고치지 않았다 — 지금 동작이 맞는 쪽이다): 창을 닫아 둬도
`NotificationsContext`의 60초 주기 확인이 계속 돈다(그 안의 `visibilityState` 검사가
셸에서는 늘 통과한다) — **배지가 닫힌 채로 움직인 이유가 이것**이고, 목록이 갱신되니
배너 판단도 제때 돈다. 그리고 `updateGate`의 `canAutoApply(risk, hidden)`은 셸에서
`hidden`이 늘 false라, `defer` 등급 업데이트는 **조용히 갈아끼우지 않고 토스트로 묻는다**
— 사용자를 놀라게 하지 않는 쪽이라 그대로 둔다.

### 밝혀 두는 트레이드오프

설치형 앱과 브라우저를 **같은 계정으로 함께 켜 두면 배너가 둘 뜬다**(앱은 이 경로,
브라우저는 웹 푸시). 일정 알림이 이미 같은 성질이고(`reminderPrefs.ts` 머리 주석),
그게 "앱/웹 구별 없이 온다"의 뒷면이다.
